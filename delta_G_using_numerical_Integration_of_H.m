...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Al2O3: 
G_al2o3 = zeros(1,length(H_al2o3));
...
for x  = 2:length(H_al2o3)
    ...
    G_al2o3(x) = (G_al2o3(x-1)*(T_b/T_a) + G); 
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Al(OH)3: 

G_aloh3 = zeros(1,length(H_al2o3));
...
for x  = 2:length(H_al2o3)
    ...
    H_b = H_aloh3(x,2);
    G = -T_b*(((T_b- T_a)/6) * ((H_a/(T_a^2)) + 4*(((H_a+ H_b)/2) / (((T_a + T_b)/2)^2)) + (H_b/(T_b^2)))); %Simpson's rule
    G_aloh3(x) = (G_aloh3(x-1)*(T_b/T_a) + G); 
end 

%AlOOH: 

G_alooh = zeros(1,length(H_alooh));
...
for x  = 2:length(H_alooh)
    ...
    H_b = H_alooh(x,2);
    G = -T_b*(((T_b- T_a)/6) * ((H_a/(T_a^2)) + 4*(((H_a+ H_b)/2) / (((T_a + T_b)/2)^2)) + (H_b/(T_b^2)))); %Simpson's rule
    G_alooh(x) = (G_alooh(x-1)*(T_b/T_a) + G); 
end 
